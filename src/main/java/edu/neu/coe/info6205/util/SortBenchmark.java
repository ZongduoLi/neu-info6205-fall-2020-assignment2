...
public class SortBenchmark {
    ...
    private void sortIntegers(final int n) {
        ...
        //2. initial array ordering situation: ordered
        final Supplier<int[]> intsSupplier2 = () -> {
            int[] result = (int[]) Array.newInstance(int.class, n);
            for (int i = 0; i < n; i++) result[i] = i;
            return result;
        };

        //3. initial array ordering situation: reversed
        final Supplier<int[]> intsSupplier3 = () -> {
            int[] result = (int[]) Array.newInstance(int.class, n);
            for (int i = 0; i < n; i++){
                result[i] = i;
            }
            int[] reverse = new int[result.length];
            for(int i=0,j=reverse.length-1;i<result.length;i++,j--){
                reverse[j] = result[i];
            }
            return reverse;
        };

        //4. initial array ordering situation: partially ordered
        final Supplier<int[]> intsSupplier4 = () -> {
            int[] result = (int[]) Array.newInstance(int.class, n);
            for (int i = 0; i < n; i++){
                result[i] = random.nextInt();
            }
            Arrays.sort(result,n/4,n/2);
            return result;
        };
        ...
        final double t1 = new Benchmark_Timer<int[]>(
                ...
                //Arrays::sort,
                InsertionSort::mutatingInsertionSort,
                null
        ).runFromSupplier(intsSupplier3, 100);
